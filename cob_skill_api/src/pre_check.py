...
class PreConditionCheck(ConditionCheck):
	...
	def init_check(self):
		...
		def check_base():
			...
			rospy.loginfo("Checking the <<base>> component resulted on %s"%self.result)			
		...
		def check_head():
			...
			if rospy.has_param('head_controler'):
				...
				self.result = "failed"
			rospy.loginfo("Checking the <<head>> component resulted on %s"%self.result)
		...
		def check_arm():
			...
			rospy.loginfo("Checking the <<arm>> component resulted on %s"%self.result)


		def check_arm_left():

			if rospy.has_param('arm_left_controller'):
				self.result = "success"
			else:
				self.result = "failed"

			rospy.loginfo("Checking the <<left arm>> component resulted on %s"%self.result)

		def check_arm_right():

			if rospy.has_param('arm_right_controller'):
				self.result = "success"
			else:
				self.result = "failed"

			rospy.loginfo("Checking the <<right arm>> component resulted on %s"%self.result)
		...
		def check_tray():
			...
			rospy.loginfo("Checking the <<tray>> component resulted on %s"%self.result)
		...
		def check_torso():
			...
			rospy.loginfo("Checking the <<torso>> component resulted on %s"%self.result)
		...
		def check_sdh():
			...
			rospy.loginfo("Checking the <<sdh>> component resulted on %s"%self.result)

		for check in self.full_components:

			try:
				{'base': check_base, 'tray': check_tray, 'torso': check_torso, 'head': check_head, 'arm': check_arm, \
					'sdh': check_sdh, 'arm_left': check_arm_left, 'arm_right': check_arm_right}[check]()

				if self.result == "failed":
					rospy.logerr("Failed when checking component <<%s>>"%check)
					return
			except KeyError:
				pass
	...
	def execute(self, userdata):	
		...
		if self.checks['init_check'] == True:
			self.init_check()

		if (self.result == "failed"):
			rospy.logerr("Check failure on <<init_check>>")
			return self.result
